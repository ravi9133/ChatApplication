		...
		if (session == null) {
			...
			try {
				...
				PreparedStatement ps = con.prepareStatement(
						"select * from frndschat_message where sender=? and trash='no' order by id desc");
				...
				while (rs.next()) {
					out.print("<tr><td>" + rs.getString("receiver") + "</td><td><a href='viewMailServlet?id="
							+ rs.getString(1) + "'>" + rs.getString("subject") + "</a></td></tr>");
				}
				out.print("</table>");
